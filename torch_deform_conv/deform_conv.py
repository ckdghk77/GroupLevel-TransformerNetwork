...
def th_batch_map_coordinates(input, coords, order=1):
    ...
    def _get_vals_by_coords(input, coords):
        ...
        inds = indices[:, 0]*input.size(2)*input.size(3)+ indices[:, 1]*input.size(2) + indices[:, 2]

        val_list = list();
        for k in range(channel_size) :
            vals = th_flatten(input[:,k,:]).index_select(0, inds)
            vals = vals.view(batch_size, n_coords)
            val_list.append(vals);

        return torch.stack(val_list,1)
    ...
    coords_offset_lt = coords - coords_lt.type(coords.data.type())
    vals_t = coords_offset_lt[..., 0].unsqueeze(1)*(vals_rt - vals_lt) + vals_lt
    vals_b = coords_offset_lt[..., 0].unsqueeze(1)*(vals_rb - vals_lb) + vals_lb
    mapped_vals = coords_offset_lt[..., 1].unsqueeze(1)* (vals_b - vals_t) + vals_t
    return mapped_vals
...
def th_batch_map_offsets(input, offsets, grid=None, order=1):
    ...
    coords = offsets + grid
    #coords[...,0] = torch.clamp(coords[...,0], min=0, max=input_height);
    #coords[...,1] = torch.clamp(coords[...,1], min=0, max=input_width);
    #print(torch.max(offsets))
    #print(torch.min(offsets))
